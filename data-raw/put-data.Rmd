...
```{r SA16_decoder}
SA16_decoder <- fread("~/Census2016.DataPack/data-raw/SA16_decoder.csv")
```
...
Mop <- function(DT, value.name = "workers", suborder = NULL, dry.run = FALSE) {
  ...
  cf_population <- 
    ...
    .[, err_r := 0] %>%
    .[ApparentPopulation > 5L, err_r := 1 - tot_workers / ApparentPopulation] %>%
    .[tot_workers > 5L, err_r := 0]
...
freadW(5) %>%
  ...
  .[CountryOfBirth %pin% "Other",
    CountryOfBirth := NA_character_] %>%
  .[, .(workers = sum(workers)),
    keyby = c(Region_key, "CountryOfBirth")] %>%
  .[]
  Mop
```
...
freadW(5) %>%
  ...
  .[, .(workers = sum(workers),
        varI = varI[1]),
    keyby = c(Region_key, "CountryOfBirth", "EmploymentStatus")] %>%
  Mop()
```
...
freadW(5) %>%
  ...
  .[, variable := sub("_Total$", "", variable)] %>%
  extract_CountryOfBirth() %>%
  .[CountryOfBirth %pin% c("Other", "Born.elsewhere"),
    CountryOfBirth := NA_character_] %>%
  .[, .(workers = sum(workers),
        varI = varI[1]),
    keyby = c(Region_key, "CountryOfBirth", "Sex")] %>%
  Mop()
```
...
freadW(5) %>%
  ...
  .[, .(workers = sum(workers),
        varI = varI[1]),
    keyby = c(Region_key, "CountryOfBirth", "EmploymentStatus", "Sex")] %>%
  Mop()
```
...
freadW(6) %>%
  ...
  .[and(variable %pin% "^[0-9]",
        variable %npin% "^3000"),
    IncomeTotPersonal.max := sub("^(1|[0-9]{3,4})_([0-9]{3,4})_.*$",
                                 "\\2",
                                 variable)] %>%
  .[variable %pin% "^3000_or_more", IncomeTotPersonal.max := "Inf"] %>%
  .[variable %pin% "Negative_Nil_income", IncomeTotPersonal.max := "0"] %>%
  .[, IncomeTotPersonal.max := round(as.double(IncomeTotPersonal.max), -1)] %>%
  .[, IncomeTotPersonal.max := 52.5 * IncomeTotPersonal.max] %>%
...
freadW(6) %>%
  ...
  .[and(variable %pin% "^[0-9]",
        variable %npin% "^3000"),
    IncomeTotPersonal.max := sub("^(1|[0-9]{3,4})_([0-9]{3,4}).*$",
                                 "\\2",
                                 variable)] %>%
  .[variable %pin% "^3000_or_more", IncomeTotPersonal.max := "Inf"] %>%
  .[variable %pin% "Negative_Nil_income", IncomeTotPersonal.max := "0"] %>%
...
freadW(6) %>%
  ...
  .[variable %pin% "Total$"] %>%
  .[, variable := sub("_Total$", "", variable)] %>%
  .[variable %pin% "^(1|[0-9]{3,4})_([0-9]{3,4})_.*$",
    IncomeTotPersonal.max := sub("^(1|[0-9]{3,4})_([0-9]{3,4}).*$",
                                 "\\2",
                                 variable)] %>%
  .[variable %pin% "^3000_or_more", IncomeTotPersonal.max := "Inf"] %>%
  .[variable %pin% "Negative_Nil_income", IncomeTotPersonal.max := "0"] %>%
  .[variable %pin% "not_stated$", IncomeTotPersonal.max := NA_character_] %>%
  .[, IncomeTotPersonal.max := round(as.double(IncomeTotPersonal.max), -1)] %>%
  .[] %>%
  Mop
```
...
freadW(6) %>%
  ...
  .[variable %pin% "Negative_Nil_income", IncomeTotPersonal.max := "0"] %>%
  .[variable %pin% "not_stated$", IncomeTotPersonal.max := NA_character_] %>%
  .[, IncomeTotPersonal.max := 52.5 * round(as.double(IncomeTotPersonal.max), -1)] %>%
  .[] %>%
  Mop
```
...
freadW(7, value.name = "workers_born_overseas") %>%
  ...
  .[IndustryOfEmployment %pin% "Inadequately described",
...
freadW(15) %>%
  ...
  splitW(EmployerType, IncomeTotPersonal.max, Occupation) %>%
  .[EmployerType != "Total"] %>%
  .[IncomeTotPersonal.max != "Total"] %>%
  .[Occupation != "Total"] %>%
  .[IncomeTotPersonal.max %ein% "Negative nil income",
    IncomeTotPersonal.max := "0"] %>%
  .[IncomeTotPersonal.max %ein% "I3000 or more",
    IncomeTotPersonal.max := "Inf"] %>%
  .[IncomeTotPersonal.max %pin% "([0-9]+)9$",
    IncomeTotPersonal.max := sub("^.* ([0-9]+)$", "\\1", IncomeTotPersonal.max)] %>%
  .[IncomeTotPersonal.max %ein% "Total personal income not stated",
    IncomeTotPersonal.max := NA_character_] %>%
  .[, IncomeTotPersonal.max := round(as.double(IncomeTotPersonal.max), -1)] %>%
  .[, IncomeTotPersonal.max := 52.5 * IncomeTotPersonal.max] %>%
  ...
  .[] %>%
  Mop(suborder = c("IncomeTotPersonal.max", "Occupation", "PublicSector", "LevelOfGovt"))
...
freadW(15) %>%
  ...
  .[, variable := sub("Negative_Nil_income", "Negative_nil_income", variable)] %>%
  .[, variable := sub("([a-z])_([0-9])", "\\1_I\\2", variable)] %>%
  splitW(EmployerType, IncomeTotPersonal.max) %>%
  .[EmployerType != "Total"] %>%
  .[IncomeTotPersonal.max != "Total"] %>%
  .[IncomeTotPersonal.max %enotin% c("Negative nil income", 
                                     "Total personal income not stated"),
    IncomeTotPersonal.max := sub("^.* ([0-9]+)$", "\\1", IncomeTotPersonal.max)] %>%
  .[IncomeTotPersonal.max %ein% "Negative nil income",
    IncomeTotPersonal.max := "0"] %>%
  .[IncomeTotPersonal.max %ein% "I3000 or more",
  ...
  .[IncomeTotPersonal.max %ein% "Total personal income not stated",
    IncomeTotPersonal.max := NA_character_] %>%
  .[, IncomeTotPersonal.max := as.double(IncomeTotPersonal.max)] %>%
  .[, IncomeTotPersonal.max := 52.5 * round(IncomeTotPersonal.max, -1)] %>%
  .[] %>%
  .[, PublicSector := grepl("government", EmployerType, ignore.case = TRUE)] %>%
  .[variable %pin% "^Not_stated", PublicSector := NA] %>%
  .[(PublicSector), LevelOfGovt := sub(" government", "", EmployerType, ignore.case = TRUE)] %>%
  ...
  .[] %>%
  Mop(suborder = c("IncomeTotPersonal.max", "PublicSector", "LevelOfGovt"))
```
...
freadW(16) %>%
  ...
  splitW(Occupation, IncomeTotPersonal.max, .Age) %>%
  .[Occupation != "Total"] %>%
  .[IncomeTotPersonal.max != "Total"] %>%
  .[.Age != "Total"] %>%
  .[IncomeTotPersonal.max %enotin% c("Negative nil income", 
...
freadW(17) %>%
  ...
  .[] %>%
  splitW(Occupation, IncomeTotPersonal.max, .HoursWorked) %>%
  .[, .HoursWorked := sub("Number of hours worked ", "", .HoursWorked)] %>%
...
freadW(18) %>%
  ...
  .[, variable := sub("3000_or_more", "I3000_or_more", variable)] %>%
  .[, variable := gsub("N(umber_of_hours_worked.*)$", "N\\L\\1\\E", variable)] %>%
  splitW(NonSchoolQualification, IncomeTotPersonal.max, Occupation) %>%
  .[Occupation != "Total"] %>%
  .[IncomeTotPersonal.max != "Total"] %>%
